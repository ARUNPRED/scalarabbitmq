...
class PGRestAPIService  {
  ...
  def pgrestget (accesstoken:String): List[Manifesto] = {
    ...
    val response = http(GET("http://localhost:3000/data"))
  ...
  def pgrestpost (accesstoken:String, data:Manifesto): Boolean = {
    ...
    val response = http(POST("http://localhost:3000/data").contentType(APPLICATION_JSON).entity(taskdata).header(AUTHORIZATION,token))
    if (response.status == OK) {
      return true
    }
    else {
